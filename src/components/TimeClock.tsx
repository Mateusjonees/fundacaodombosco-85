...
import { Clock, LogIn, LogOut, Coffee, Play, Camera } from 'lucide-react';
...
export function TimeClock() {
  ...
  const calculateTotalHours = (entry: TimesheetEntry): number => {
    ...
    if (entry.break_start && entry.break_end) {
      totalMinutes -= differenceInMinutes(new Date(entry.break_end), new Date(entry.break_start));
    }
    return totalMinutes / 60;
  };

  const uploadPhoto = async (blob: Blob, action: string): Promise<string | null> => {
    if (!user?.id) return null;
    const fileName = `${user.id}/${format(new Date(), 'yyyy-MM-dd')}_${action}_${Date.now()}.jpg`;
  ...
  const openCameraFor = (action: ClockAction) => {
    ...
    try {
      ...
      switch (pendingAction) {
        case 'clock_in': {
          ...
          if (error) throw error;
          toast({ title: "Entrada registrada", description: `Ponto batido às ${format(now, 'HH:mm:ss')} com foto` });
          break;
        }
        case 'clock_out': {
          if (!currentEntry) return;
          const totalHours = calculateTotalHours({ ...currentEntry, clock_out: now.toISOString() });
          const { error } = await supabase
            .from('employee_timesheet')
            .update({ clock_out: now.toISOString(), total_hours: totalHours, clock_out_photo_url: photoPath })
            .eq('id', currentEntry.id);
          if (error) throw error;
          toast({ title: "Saída registrada", description: `Ponto batido às ${format(now, 'HH:mm:ss')}. Total: ${totalHours.toFixed(2)}h` });
          break;
        }
        case 'break_start': {
          if (!currentEntry) return;
          const { error } = await supabase
            .from('employee_timesheet')
            .update({ break_start: now.toISOString(), break_start_photo_url: photoPath })
            .eq('id', currentEntry.id);
          if (error) throw error;
          toast({ title: "Pausa iniciada", description: `Início da pausa às ${format(now, 'HH:mm:ss')}` });
          break;
        }
        case 'break_end': {
          if (!currentEntry) return;
          const { error } = await supabase
            .from('employee_timesheet')
            .update({ break_end: now.toISOString(), break_end_photo_url: photoPath })
            .eq('id', currentEntry.id);
          if (error) throw error;
          toast({ title: "Pausa finalizada", description: `Fim da pausa às ${format(now, 'HH:mm:ss')}` });
          break;
        }
      }
      ...
      toast({ variant: "destructive", title: "Erro", description: error.message || "Não foi possível registrar o ponto." });
    } finally {
      setLoading(false);
      setPendingAction(null);
    }
  }, [pendingAction, user, currentEntry, queryClient, toast]);
  ...
  const getWorkingTime = () => {
    ...
    if (currentEntry.break_start) {
      ...
      totalMinutes -= differenceInMinutes(breakEnd, new Date(currentEntry.break_start));
    }
  ...
  return (
    <>
      <Card>
        ...
        <CardContent className="space-y-4">
            ...
            <div className="space-y-2 text-sm">
              {currentEntry.clock_in && (
                <div className="flex justify-between items-center p-2 bg-muted/50 rounded">
                  <span className="text-muted-foreground flex items-center gap-1">
                    Entrada:
                    {currentEntry.clock_in_photo_url && <Camera className="h-3 w-3 text-green-500" />}
                  </span>
              ...
              {currentEntry.clock_out && (
                <div className="flex justify-between items-center p-2 bg-muted/50 rounded">
                  <span className="text-muted-foreground flex items-center gap-1">
                    Saída:
                    {currentEntry.clock_out_photo_url && <Camera className="h-3 w-3 text-green-500" />}
                  </span>
          ...
          <div className="space-y-2">
            {!hasClockIn && (
              <Button onClick={() => openCameraFor('clock_in')} disabled={loading} className="w-full" size="lg">
                <Camera className="mr-2 h-4 w-4" />
                Registrar Entrada (com foto)
              </Button>
            )}
            ...
            {hasClockIn && !hasClockOut && (
              <>
                {!currentEntry?.break_start && (
                  <Button onClick={() => openCameraFor('break_start')} disabled={loading} variant="outline" className="w-full">
                ...
                {currentEntry?.break_start && !currentEntry?.break_end && (
                  <Button onClick={() => openCameraFor('break_end')} disabled={loading} variant="outline" className="w-full">
                    <Play className="mr-2 h-4 w-4" />
                    Finalizar Pausa
                  </Button>
                )}

                <Button onClick={() => openCameraFor('clock_out')} disabled={loading || !!isOnBreak} variant="default" className="w-full" size="lg">
                  <Camera className="mr-2 h-4 w-4" />
                  Registrar Saída (com foto)
                </Button>
              </>
            )}
      ...
      <CameraCapture
        ...
        title={
          pendingAction === 'clock_in' ? 'Foto de Entrada' :
          pendingAction === 'clock_out' ? 'Foto de Saída' :
          pendingAction === 'break_start' ? 'Foto - Início Pausa' :
          pendingAction === 'break_end' ? 'Foto - Fim Pausa' :
          'Tire sua foto'
        }
      />
    </>
