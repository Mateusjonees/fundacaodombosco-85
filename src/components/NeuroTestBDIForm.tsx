...
import { calculateBDIResults, type BDIResults } from '@/data/neuroTests/bdi';

interface Props {
  patientAge: number;
  onResultsChange: (results: BDIResults | null) => void;
  onRemove?: () => void;
}

export default function NeuroTestBDIForm({ patientAge, onResultsChange, onRemove }: Props) {
  ...
  useEffect(() => {
    ...
    if (!isNaN(score) && score >= 0 && score <= 63) {
      onResultsChange(calculateBDIResults(score));
    } else {
  ...
  return (
    <Card className="border-primary/20">
      <CardHeader className="p-3 pb-2">
        ...
        <p className="text-xs text-muted-foreground">Avalia intensidade de sintomas depressivos • 13-80 anos</p>
      </CardHeader>
      <CardContent className="p-3 pt-0 space-y-3">
        ...
        <Collapsible open={showInstructions} onOpenChange={setShowInstructions}>
          ...
          <CollapsibleContent>
            <div className="mt-2 p-2.5 bg-muted/40 rounded-lg text-xs text-muted-foreground space-y-1">
              <p>• O paciente deve escolher a alternativa (0-3) que melhor descreve como se sentiu nas <strong>últimas duas semanas</strong>.</p>
              <p>• São 21 itens, cada um com 4 alternativas (0 a 3).</p>
              <p>• Some todos os valores marcados para obter o escore total (0-63).</p>
        ...
        {/* Resultado */}
        {isValid && (() => {
          const results = calculateBDIResults(score);
          return (
